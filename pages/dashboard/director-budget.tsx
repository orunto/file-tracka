...
export const getStaticProps: GetStaticProps = async () => {
    const assigned = await prisma.filerecords.findMany({
        where: { actionTaken: 'Appraised', fileLocation: 'Group' },
        select: {
            mda: true,
            assignedGroup: true,
            fileTitle: true,
    ...
    const queue = await prisma.filerecords.findMany({
        where: { actionTaken: 'Appraised', fileLocation: 'DB' },
        select: {
            mda: true,
            assignedGroup: true,
            fileTitle: true,
    ...
    const returned = await prisma.filerecords.findMany({
        where: { actionTaken: 'Returned', fileLocation: 'DB', },
        select: {
            mda: true,
            assignedGroup: true,
            fileTitle: true,
...
export default function Groups(props: Props) {
    ...
    const [dbaccept, setdbAccept] = useState('flex')
    ...
    if (user) {
        if (user.sub == process.env.AUTH0_DIRECTOR_ID || process.env.AUTH0_SUPER) {
            return (
                <>
                            ...
                            <section className='flex flex-col items-end w-full box-border gap-8'>
                                {
                                    worktable && (
                                        <>
                                        <TablesToggle clickeventone={() => { setLine("0%"); setButtonColor("button_one"); setRows([]); setRows(props.assigned); setdbAccept('flex'); setView('none'); setdbAction('none'); setHeader([`MDA`, `Group`, `File Title`, `File Number`, `Amount`, `Date`, `Action Taken`, ``]) }}
                                            clickeventtwo={() => { setLine("100%"); setButtonColor("button_two"); setRows([]); setRows(props.queue); setdbAccept('none'); setdbAccept('none'); setdbAction('flex'); setView('none'); setHeader([`MDA`, `Group`, `File Title`, `File Number`, `Amount`, `Date`, `Days`, `Action `, ``]) }}
                                            clickeventthree={() => { setLine("200%"); setButtonColor("button_three"); setRows([]); setRows(props.returned); setdbAccept('none'); setdbAction('none'); setView('flex'); setHeader([`MDA`, `Group`, `File Title`, `File Number`, `Amount`, `Date`, `Action Taken`, ``]) }}
                                            clickeventfour={() => { setLine("300%"); setButtonColor("button_four"); setRows([]); setRows(props.track); setdbAccept('none'); setdbAction('none'); setView('flex'); setHeader([`MDA`, `Group`, `File Title`, `File Number`, `Amount`, `Date`, `Action Taken`, ``]) }}
                                            buttoncolor={buttonColor} line={line}
                                            buttonone={`Reception`} buttontwo={`Queue`} buttonthree={`Returned`} buttonfour={`Recommended`}
                                        />
        
                                        <Table viewbutton={view} actionbutton={action} acceptbutton={accept} pageno={pageNo} content={rows} headers={header} actions={content.Actions.Registry} dbacceptbutton={dbaccept} psacceptbutton={psaccept} comacceptbutton={comaccept} filelocation={'DB'} dbactionbutton={dbaction} psactionbutton={'none'} comactionbutton={'none'} dbviewbutton={dbview} psviewbutton={psview} comviewbutton={comview} rejviewbutton={rejview} finish={'none'} />
                                        </>
                                    )
                                }

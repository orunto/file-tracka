...
export default function Registry() {
    ...
    function getAssigned() {
        fetch('/api/get/assigned/group/A').then(response => response.json()).then((result) => setRows(result))
    }
    ...
    function getQueue() {
        fetch('/api/get/queue/group/A').then(response => response.json()).then((result) => setRows(result))
    }
            ...
            return (
                <main className='box-border flex w-screen h-screen overflow-hidden relative'>
                    ...
                    <section className='box-border flex flex-col w-full gap-16 relative h-full overflow-x-hidden'>
                        <Header user_role={`Beta Tester`} username={user.name} page={`Group A`} notification={notifications} />

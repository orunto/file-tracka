...
export const getStaticProps: GetStaticProps = async () => {
    const assigned = await prisma.filerecords.findMany({
        where: { actionTaken: 'Assigned', assignedGroup: 'A', fileLocation: 'Registry' },
        select: {
            mda: true,
            fileTitle: true,
            fileNumber: true,
    ...
    const queue = await prisma.filerecords.findMany({
        where: { actionTaken: 'Assigned', assignedGroup: 'A', fileLocation: 'Group' },
        select: {
            mda: true,
            fileTitle: true,
            fileNumber: true,
    ...
    const completed = await prisma.filerecords.findMany({
        where: { actionTaken: 'Approved', assignedGroup: 'A', fileLocation: 'Group' },
        select: {
            mda: true,
            fileTitle: true,
            fileNumber: true,
    ...
    const returned = await prisma.filerecords.findMany({
        where: { actionTaken: 'Returned', assignedGroup: 'A', },
        select: {
            mda: true,
            fileTitle: true,
            fileNumber: true,
    ...
    const track = await prisma.filerecords.findMany({
        where: { actionTaken: 'Appraised', assignedGroup: 'A', },
        select: {
            mda: true,
            fileTitle: true,
            fileNumber: true,
...
export default function Groups(props: Props) {
    ...
    const [header, setHeader] = useState([`MDA`, `File Title`, `File Number`, `Amount`, `Date`, `Action Taken`, ``])
    ...
    if (user) {
        if (user.sub == process.env.AUTH0_GROUPA_ID || process.env.AUTH0_SUPER) {
            return (
                <>
                            ...
                            <section className='flex flex-col items-end w-full box-border gap-8'>
                                {
                                    worktable && (
                                        <>
                                            ...
                                            <TablesToggle clickeventone={() => { setLine("0%"); setButtonColor("button_one"); setRows([]); setRows(props.assigned); setAction('none'); setView('none'); setAccept('flex'); setHeader([`MDA`, `File Title`, `File Number`, `Amount`, `Date`, `Action Taken`, ``]) }}
                                                clickeventtwo={() => { setLine("100%"); setButtonColor("button_two"); setRows([]); setRows(props.queue); setAccept('none'); setAction('flex'); setView('none'); setHeader([`MDA`, `File Title`, `File Number`, `Amount`, `Date`, `Days`, `Action`, ``]) }}
